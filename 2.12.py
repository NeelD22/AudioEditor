...
def get_transcription_result_url(audio_url):
    ...
    while True:
        ...
        if data.json()['status'] == "completed":
            ...
            # confidence obtainer!
            mydict = {}
            for i in words:  # create a dict which associates word w confid
                word = i.get("text")
                start = i.get("start")
            ...
            for block in indexer:
                ...
                # Character Tag line
                block.append(character_index)

            return indexer, highlighted_words
...
def file_creator(indexer):
    file = open("TranscriptVol4.txt", 'a')
    speaker_check = 'A'
    for block in indexer:
        print('block lah ', block)
        if speaker_check == block[4]:  # same speaker
            file.write(" " + block[0] + " ")
            speaker_check = block[4]
        else:  # new speaker
            file.write('\n' + '\n')
            file.write(" " + block[0] + " ")
            speaker_check = block[4]
...
highlighted_words, indexer = get_transcription_result_url(audio_url)
#text_displayer(linearray)
file_creator(indexer)
